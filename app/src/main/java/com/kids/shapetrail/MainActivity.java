...
public class MainActivity extends Activity implements TextToSpeech.OnInitListener {
  ...
  private final String[] shapes={"Circle","Square","Triangle","Rectangle","Oval","Diamond","Star","Heart","Pentagon","Hexagon","Octagon","Crescent","Cross","Arrow","Trapezoid","Parallelogram","Cloud","Sun","Flower","Spiral"};
  private final String[] sides={"no sides — it is round","4 sides","3 sides","4 sides","no sides — it is round","4 sides","10 points","no sides — it is curved","5 sides","6 sides","8 sides","no sides — it is curved","12 sides","7 sides","4 sides","4 sides","no sides — it is curved","many sun rays","many soft petals","no sides — it curls around"};
  ...
  void build(){
    ...
    count=label("Shape "+(current+1)+" of 20  •  " + sides[current],16);root.addView(count,new LinearLayout.LayoutParams(-1,dp(32)));
  ...
  void next(){handler.removeCallbacksAndMessages(null);current=(current+1)%shapes.length;title.setText("Trace the " + shapes[current]);count.setText("Shape "+(current+1)+" of 20  •  " + sides[current]);trace.setShape(current);speakShape();}
  void completed(){chime.startTone(ToneGenerator.TONE_DTMF_9,120);speak("Wonderful tracing! " + shapes[current] + "!");handler.postDelayed(()->next(),2600);}
  ...
  class TraceView extends View {
    ...
    void clear(){drawing.reset();done=false;invalidate();}
    protected void onDraw(Canvas c){super.onDraw(c); float w=getWidth(),h=getHeight();c.drawRoundRect(dp(6),dp(6),w-dp(6),h-dp(6),dp(28),dp(28),fill); target.reset(); make(target,w,h);c.save();c.rotate(toyTurn,w/2,h/2+dp(20));toy.setStyle(Paint.Style.STROKE);toy.setStrokeWidth(dp(18));toy.setStrokeCap(Paint.Cap.ROUND);toy.setColor(Color.argb(38,255,148,103));c.drawPath(target,toy);c.restore();c.drawPath(target,guide);c.drawPath(drawing,ink); if(done){Paint p=new Paint(1);p.setColor(Color.rgb(45,160,105));p.setTextSize(dp(25));p.setTypeface(Typeface.DEFAULT_BOLD);p.setTextAlign(Paint.Align.CENTER);c.drawText("Wonderful tracing! ★",w/2,dp(42),p);}}
